...
async function getHash(url) {
  try {
    ...
    logger.debug({ url }, 'Could not generate hash for url');
    return null;
  }
}
...
async function updateDependency(fileContent, upgrade) {
  try {
    ...
    if (
      ...
      let url = `https://github.com/${upgrade.repo}/releases/download/${upgrade.newValue}/${shortRepo}-${upgrade.newValue}.tar.gz`;
      hash = await getHash(url);
      if (!hash) {
        url = `https://github.com/${upgrade.repo}/archive/${upgrade.newValue}.tar.gz`;
        hash = await getHash(url);
        if (!hash) {
          logger.warn('Could not generate hash for bazel http_archive update');
          return null;
        }
      }
